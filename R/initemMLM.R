...
initemMLM <- function(X, Xlogit, Y, MLMoption) {
  ...
  for (j in 1:numcmp) {
    if (ndatpercls[j] > 0) {
      sigma_j <- as.matrix(sigmainit[, , j] / ndatpercls[j])
      sigmainit[, , j] <- sigma_j
    }
    # Force matrix format to prevent dimension drop for 1x1 case
    sigma_j <- as.matrix(sigmainit[, , j])
    sigmainit[, , j] <- checksingular(sigma_j, Xvar, 0.05)
  }
  ...
  for (j in 1:numcmp) {
    ...
    if (min_count >= 2) {
      ...
      lasso_result <- glmnet(Xsmall, Ysmall, family = MLMoption$DISTR, lambda = MLMoption$lambdaLasso[j], alpha = MLMoption$AlphaLasso)
